...
function addMaterial() {
    ...
    // Check if this is a paper option (value starts with "paper_")
    const isPaper = materialId.startsWith('paper_');
    console.log(`Selected: ${materialId} - ${materialText}, Quantity: ${quantity}, isPaper: ${isPaper}`);
    ...
    if (!exists) {
        ...
        // If it's a paper option, add special attributes
        if (isPaper) {
            const selectedOption = materialSelect.options[materialSelect.selectedIndex];
            const paperId = selectedOption.getAttribute('data-paper-id');
            const paperName = selectedOption.getAttribute('data-paper-name');
            ...
            materialItem.innerHTML = `
                ...
                <input type="hidden" name="template_material_paper_id[]" value="${paperId}">
                <div>
    ...
    document.querySelectorAll('input[name="template_material_ids[]"]').forEach((input, index) => {
        ...
        const isPaper = document.querySelectorAll('input[name="template_material_is_paper[]"]')[index]?.value === 'true';
        console.log(`- Material ID: ${input.value}, Quantity: ${qtyInput.value}, isPaper: ${isPaper}`);
    });
}
