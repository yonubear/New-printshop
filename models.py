...
class Quote(db.Model):
    ...
    def get_discount_amount(self):
        """Calculate discount amount based on customer's discount percentage"""
        if not self.customer or self.customer.discount_percentage <= 0:
            return 0.0
        subtotal = self.get_subtotal()
        return subtotal * (self.customer.discount_percentage / 100.0)
    
    def get_total(self):
        """Calculate quote total after discount"""
        subtotal = self.get_subtotal()
        discount = self.get_discount_amount()
        return subtotal - discount
    
    def update_total(self):
        """Update the quote's total_price field based on items"""
